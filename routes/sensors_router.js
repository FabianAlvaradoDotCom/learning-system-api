...
router.post("/create-sensor-reading", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    req.body.sensors.forEach(async (sent_sensor, index) => {
      ...
      if(+sent_sensor.output_data < found_alert_limit.alert_minimum_value ){
        ...
        if(found_alert_limit.alert_trigger_status !== "lower" && found_alert_limit.alert_enable_status === true){
          //await emailAlerts(found_alert_limit.alert_distribution_list, `The sensor ${sent_sensor.sensor_name} received data that is below the expected range of possible conditions. You are receiving this because you are in the list of recipients to be notified on issues.`);
        ...
        if(found_alert_limit.alert_trigger_status !== "higher" && found_alert_limit.alert_enable_status === true){
          // await emailAlerts(found_alert_limit.alert_distribution_list, `The sensor ${sent_sensor.sensor_name} received data that is above the expected range of possible conditions. You are receiving this because you are in the list of recipients to be notified on issues.`);
          // Instead of sending an email for each sensor, we attach the new the found email with alerts so we send just an email.
      ...
      // console.log(html_alert_email_body);
        ...
        if(html_alert_email_body.length > 1){
    ...
    // Before sending the response, we send the email with problematic sensors:
    //console.log("Fuera", html_alert_email_body.length);
    //console.log("Fuera", html_alert_email_body);
